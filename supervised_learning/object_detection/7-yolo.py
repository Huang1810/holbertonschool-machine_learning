#!/usr/bin/env python3
"""
Yolo class for complete object detection pipeline using YOLOv3 algorithm.
"""
...
class Yolo:
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        ...
        for box, cls, score in zip(boxes, box_classes, box_scores):
            ...
            cv2.rectangle(img, (x1, y1-20), (x1+text_w, y1), (255, 0, 0), -1)
    ...
    def predict(self, folder_path):
        ...
        for i, (image, shape) in enumerate(zip(images, image_shapes)):
            # Process outputs for single image
            single_outputs = [outputs[j][i:i+1] for j in range(len(outputs))]
            boxes, confidences, class_probs = self.process_outputs(single_outputs, shape)
            boxes, classes, scores = self.filter_boxes(boxes, confidences, class_probs)
            boxes, classes, scores = self.non_max_suppression(boxes, classes, scores)
            ...
            self.show_boxes(image, boxes, classes, scores, filename)
